...
class _NavItem(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self._active:
            ...
            text_color = QColor(theme.color("text_dim"))
...
class _RecentItem(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # 文件名
        p.setPen(QColor(theme.color("text_dim") if not self._hovered else theme.color("text")))
        font = p.font()
...
class PlaylistSidebar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # 视频库分组
        self._section_label = QLabel("视频库")
        nav_layout.addWidget(self._section_label)
        ...
        # 设置分组
        self._settings_label = QLabel("设置")
        nav_layout.addWidget(self._settings_label)
        ...
        self._theme_item = _NavItem("theme", "浅色模式" if theme.is_dark else "深色模式",
                                     "sun" if theme.is_dark else "moon")
        self._theme_item.clicked.connect(lambda _: self._on_theme_toggle())
        nav_layout.addWidget(self._theme_item)
        ...
        # 版本号
        self._version_label = QLabel(f"v {get_version()}")
        self._version_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        nav_layout.addWidget(self._version_label)
        ...
        theme.theme_changed.connect(self._update_theme_item)

    def _on_theme_toggle(self) -> None:
        theme.toggle()

    def _update_theme_item(self) -> None:
        self._theme_item._label = "浅色模式" if theme.is_dark else "深色模式"
        self._theme_item._icon_type = "sun" if theme.is_dark else "moon"
        self._theme_item.update()

    def _apply_theme(self) -> None:
        self.setStyleSheet(
            f"background: {theme.color('panel')}; border-right: 1px solid {theme.color('border_half')};"
        )
        ...
        """
        self._section_label.setStyleSheet(section_style)
        self._settings_label.setStyleSheet(section_style)
